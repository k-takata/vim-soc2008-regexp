/* pathdef.c */
/* This file is automatically created by Makefile
 * DO NOT EDIT!  Change Makefile only. */
#include "vim.h"
char_u *default_vim_dir = (char_u *)"/usr/local/share/vim";
char_u *default_vimruntime_dir = (char_u *)"";
char_u *all_cflags = (char_u *)"gcc -c -I. -Iproto -DHAVE_CONFIG_H     -g -fno-strength-reduce -Wall -DDEBUG        ";
char_u *all_lflags = (char_u *)"gcc   -L/usr/local/lib -o vim       -lncurses  -lacl ";
char_u *compiled_user = (char_u *)"andrei";
char_u *compiled_sys = (char_u *)"jubileum";
